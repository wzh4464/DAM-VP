...
class EuroSAT(ImageFolder):
    ...
    def __init__(
        ...
        self.root = os.path.expanduser(args.data_dir)
        ...
        super().__init__(
            self._data_folder, 
            transform=get_transforms("train", args.crop_size, args.pretrained_model), 
            target_transform=target_transform
        )
        self.root = os.path.expanduser(args.data_dir)
...
if __name__ == '__main__':
    ...
    for sample in DataLoader(dataset_train, batch_size=32):
        if isinstance(sample, dict):
            ...
            logger.info(sample[0].shape)
            logger.info(sample[1].shape)
        else:
