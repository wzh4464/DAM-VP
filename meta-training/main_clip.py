...
def load_meta_dataset(args):
    ...
    for i in range(args.num_tasks):
        mini, class_names = data_loader.construct_train_loader(args, args.meta_datasets[i])
        minis.append(mini)
        minis_class_names[args.meta_datasets[i]] = refine_classname(class_names)
    # load test
    minis_test_trainset, minis_test_class_names = data_loader.construct_train_loader(args, args.test_dataset)
    minis_test_class_names = refine_classname(minis_test_class_names)
    minis_test = [
        minis_test_trainset, 
        data_loader.construct_val_loader(args, args.test_dataset)[0], 
        data_loader.construct_test_loader(args, args.test_dataset)[0]
    ]
    return minis, minis_test, minis_class_names, minis_test_class_names
...
def main():
    ...
    minis, minis_test, minis_class_names, minis_test_class_names = load_meta_dataset(args)
    ...
    def save_prompter(prompter, acc, epoch):
        file_path = "./checkpoints_clip_vit_b/" + "lr" + str(args.meta_lr) + \
    ...
    if not args.wo_da:
        # coarse clustering
        metalearner.coarse_clustering(minis, minis_class_names, "meta_training")
        metalearner.coarse_clustering(minis_test, minis_test_class_names, "task_adapting")
    ...
    for epoch in range(args.epochs):
        ...
        if epoch == 0:
            ...
            accs = metalearner.finetuning(minis_test, minis_test_class_names)
        ...
        for step in range(step_number):
            ...
            accs = metalearner(batch_data, minis_class_names)
            if (step + 1) % 1 == 0:
         ...
         # evaluation
        if (epoch + 1) % 1 == 0:
            accs = metalearner.finetuning(minis_test, minis_test_class_names, BEST_PROMPTER)
            test_acc_list.append(accs)
            logger.info('[Prompt Finetuning] Testing acc on {}: {}'.format(args.test_dataset, accs))
